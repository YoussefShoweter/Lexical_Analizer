...
def GOTO(state, charNextToDot):
    ...
    if stateExists == -1:
        ...
        # assign that previous state NUM
...
def createParseTable(statesDict, stateMap, T, NT):
    ...
    # NUM the separated rules
    NUMed = {}
    key_count = 0
    for rule in seperatedRulesList:
        tempRule = copy.deepcopy(rule)
        tempRule[1].remove('.')
        NUMed[key_count] = tempRule
        key_count += 1
...
nonterm_userdef = ['lexp', 'atom', 'list','lexp-seq']
...
#          "SQ -> SQ S | S",
#          "S -> IfS | AssS",
#          "IfS -> if NUM then SQ end",
#          "AssS -> ID := factor ;",
#          "factor -> ID | NUM"
#          ]
# nonterm_userdef = ['SQ', 'S', 'IfS','AssS','factor']
# term_userdef = [ 'ID', 'NUM',':=',';','if','then','end']
# start_symbol = nonterm_userdef[0]
